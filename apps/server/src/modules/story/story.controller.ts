...
export class StoryController {
  ...
  getActiveStories(@Req() req: AuthenticatedRequest, @Query("vendorId") vendorId?: string) {
    return this.storyService.getActiveStories(req.user.id, vendorId);
  }

  @Post()
  @ApiOperation({ summary: "Create a new story" })
  createStory(@Req() req: AuthenticatedRequest, @Body() body: CreateStoryDto) {
    return this.storyService.createStory(req.user.id, body);
  }

  @Delete(":id")
  @ApiOperation({ summary: "Delete a story" })
  deleteStory(@Req() req: AuthenticatedRequest, @Param("id") id: string) {
    return this.storyService.deleteStory(req.user.id, id);
  }

  @Post(":id/view")
  @ApiOperation({ summary: "Record a story view" })
  recordView(@Req() req: AuthenticatedRequest, @Param("id") id: string) {
