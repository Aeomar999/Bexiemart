...
jest.mock("../stores/auth-store", () => ({
  useAuthStore: { getState: jest.fn(() => ({ logout: jest.fn() })) },
}));
...
describe("apiClient", () => {
  const mockedClient = apiClient as unknown as MockedClient;
  ...
  it("should set auth header on requests", async () => {
    ...
    const handler = mockedClient.interceptors.request.use.mock.calls[0][0];
  ...
  it("should handle 401 by rejecting", async () => {
    const errorHandler = mockedClient.interceptors.response.use.mock.calls[0][1];
