...
export const walletApi = {
  ...
  initializeTopUp: (amount: number, channel: string) =>
    apiClient.post("/wallet/topup/initialize", { amount, channel }),
  verifyTopUp: (reference: string) => apiClient.get(`/wallet/topup/verify/${reference}`),
  transfer: (recipientEmail: string, amount: number, pin: string) =>
    apiClient.post("/wallet/transfer", { recipientEmail, amount, pin }),
  withdraw: (payload: {
    amount: number;
    accountId: string;
  ...
  changePin: (currentPin: string, newPin: string) =>
    apiClient.post("/wallet/pin/change", { currentPin, newPin }),
  ...
  verifyAndSaveCard: (data: { reference: string }) =>
    apiClient.post("/wallet/cards/verify-save", data),
