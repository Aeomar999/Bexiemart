...
describe("usePaymentMethods", () => {
  ...
  it("should list payment methods on mount", async () => {
    (customerPaymentMethodsApi.getAll as jest.Mock).mockResolvedValue({
      data: { data: [{ id: "pm1", type: "card", provider: "visa" }] },
    });
  ...
  it("should handle empty methods", async () => {
    (customerPaymentMethodsApi.getAll as jest.Mock).mockResolvedValue({ data: { data: [] } });
    const { result } = renderHook(() => usePaymentMethods(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isPending).toBeFalsy());
...
describe("useInitializePayment", () => {
  ...
  it("should call initialize with orderId", async () => {
    (paymentsApi.initialize as jest.Mock).mockResolvedValue({
      data: { url: "https://pay.com/checkout" },
    });
