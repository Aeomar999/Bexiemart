...
describe("useFoodRestaurants", () => {
  ...
  it("should fetch food restaurants on mount", async () => {
    (foodApi.getRestaurants as jest.Mock).mockResolvedValue({
      data: { data: [{ id: "r1", name: "Pizza Place" }] },
    });
    const { result } = renderHook(() => useFoodRestaurants({ category: "pizza" }), {
...
describe("useFoodItems", () => {
  ...
  it("should fetch food items with search filter", async () => {
    (foodApi.getItems as jest.Mock).mockResolvedValue({
      data: { data: [{ id: "i1", name: "Margherita", price: 1500 }] },
    });
    const { result } = renderHook(() => useFoodItems({ search: "margherita" }), {
      wrapper: createWrapper(),
    });
    await waitFor(() => expect(result.current.isPending).toBeFalsy());
...
describe("useFoodCart / useAddToFoodCart", () => {
  ...
  it("should fetch food cart", async () => {
    (foodApi.getCart as jest.Mock).mockResolvedValue({
      data: { items: [{ id: "ci1", name: "Margherita", quantity: 2 }] },
    });
    const { result } = renderHook(() => useFoodCart(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isPending).toBeFalsy());
    expect(result.current.data).toEqual({
      items: [{ id: "ci1", name: "Margherita", quantity: 2 }],
    });
  });
  ...
  it("should add item to cart mutation", async () => {
    ...
    expect(foodApi.addToCart).toHaveBeenCalledWith({
      foodItemId: "i1",
      quantity: 2,
      specialInstructions: "Extra cheese",
    });
...
describe("useFoodCheckout / useFoodOrders", () => {
  ...
  it("should fetch food orders list", async () => {
    (foodApi.getOrders as jest.Mock).mockResolvedValue({
      data: { data: [{ id: "o1", status: "pending" }] },
    });
