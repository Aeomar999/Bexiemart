...
export function Badge({
  ...
  if (variant === "count") {
    ...
    return (
      <View className="absolute -top-1 -right-1 min-w-[20] h-5 rounded-full bg-accent-600 items-center justify-center px-1.5">
  ...
  if (variant === "status" && label) {
    ...
    return (
      <View className={`px-3 py-1 rounded-full ${colors.bg}`}>
        <Text className={`text-caption font-semibold font-body ${colors.text}`}>
          {label}
        </Text>
      </View>
    );
  ...
  if (variant === "discount" && label) {
    return (
      <View className="absolute top-2 left-2 px-2 py-0.5 rounded bg-accent-600">
        <Text className="text-caption font-bold text-white font-body">
          {label}
        </Text>
      </View>
