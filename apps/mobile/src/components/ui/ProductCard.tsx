...
export function ProductCard({
      ...
      >
        ...
        <View className="flex-row justify-between items-center px-1">
          <View className="flex-row items-center">
            <Text className="text-[12px] font-bold text-foreground">GHS {numericPrice.toFixed(0)}</Text>
  ...
  if (variant === "horizontal") {
    return (
      <Pressable style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]} 
        className="w-full flex-row active:opacity-70 bg-card p-3 rounded-[20px] border border-border mb-3"
        onPress={onPress}
      >
        ...
        <View className="flex-1 justify-center">
          <View className="flex-row justify-between items-start mb-1">
            <Text className="text-[15px] font-bold text-foreground flex-1 pr-2" numberOfLines={1}>{name}</Text>
            {onFavoriteToggle && (
              <Pressable 
          ...
          {subtitle && <Text className="text-[12px] text-muted-foreground font-body mb-2" numberOfLines={1}>{subtitle}</Text>}
          <View className="flex-row items-center justify-between mt-auto">
            <View className="flex-row items-center gap-1.5">
              <Text className="text-[15px] font-bold text-foreground">GHS {numericPrice.toFixed(2)}</Text>
              {numericOldPrice && numericOldPrice > numericPrice && (
                <Text className="text-[11px] text-muted-foreground line-through">GHS {numericOldPrice.toFixed(2)}</Text>
              )}
            </View>
            {rating && (
  ...
  return (
    ...
    >
      ...
      <Text className="text-[14px] font-bold text-foreground" numberOfLines={1}>{name}</Text>
      {subtitle && <Text className="text-[11px] text-muted-foreground font-body mb-1" numberOfLines={1}>{subtitle}</Text>}
      <View className="flex-row items-center gap-1.5">
        <Text className="text-[14px] font-bold text-foreground">GHS {numericPrice.toFixed(2)}</Text>
        {numericOldPrice && numericOldPrice > numericPrice && (
          <Text className="text-[10px] text-muted-foreground line-through">GHS {numericOldPrice.toFixed(2)}</Text>
        )}
      </View>
    </Pressable>
  );
