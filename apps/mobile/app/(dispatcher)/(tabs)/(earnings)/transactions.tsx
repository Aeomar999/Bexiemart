...
export default function TransactionsScreen() {
  ...
  const filteredTransactions = transactions?.filter((t: any) => filter === "all" || t.type === filter) || [];
    ...
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top }}>
      ...
      <View className="px-5 py-4 flex-row gap-2">
        {(["all", "order", "withdrawal"] as const).map((f) => (
          ...
          >
            <Text className={`text-[14px] font-bold capitalize ${filter === f ? 'text-background' : 'text-muted-foreground'}`}>
              {f === 'order' ? 'Earnings' : f}
            </Text>
          </Pressable>
        ))}
      ...
      <ScrollView className="flex-1 px-5" contentContainerClassName="pb-24">
        {isLoading ? (
          ...
          <View className="py-20 items-center justify-center">
            ...
            <Text className="text-[16px] font-bold text-foreground mb-1">No Transactions Found</Text>
            <Text className="text-[14px] text-muted-foreground text-center">
              {filter === "all" ? "You haven't made any transactions yet." : `You have no ${filter === 'order' ? 'earning' : filter} transactions.`}
            </Text>
          </View>
        ) : (
