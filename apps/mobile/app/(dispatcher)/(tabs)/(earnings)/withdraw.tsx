...
export default function WithdrawFundsScreen() {
      ...
      },
      {
        ...
        onError: (error: any) => {
          ...
          showPopup({
            ...
            message:
              error?.response?.data?.message || error?.message || "Check your PIN and try again.",
          });
        },
      }
    );
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      {methods.length === 0 ? (
        <View
          className="m-5 p-5 bg-card border border-border rounded-2xl items-center"
          style={{ marginTop: (insets.top || 12) + 80 }}
        >
