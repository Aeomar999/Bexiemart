...
export default function WithdrawFundsScreen() {
  ...
  const handleWithdrawRequest = () => {
    if (!amount || numAmount <= 0) {
      showPopup({ type: "error", title: "Invalid Amount", message: "Please enter a valid amount to withdraw." });
      return;
    }

    if (totalDeduction > availableBalance) {
      showPopup({ type: "error", title: "Insufficient Funds", message: "The total deduction exceeds your available balance." });
      return;
    }
  ...
  const handleMaxAmount = () => {
    const maxAmount = Math.max(0, availableBalance - 5.00);
  ...
  return (
    <View className="flex-1 bg-surface-50">
      ...
      >
        ...
        <View className="bg-white rounded-[32px] p-6 mb-8 border border-surface-100 shadow-sm shadow-surface-200/50 mt-4">
          <View className="flex-row justify-between items-center mb-6">
            <View className="flex-row items-center bg-brand-50 px-3 py-1.5 rounded-full border border-brand-100">
              ...
              <Text className="text-[12px] font-bold text-brand-700">Available: GHS {availableBalance.toFixed(2)}</Text>
            </View>
            <Pressable onPress={handleMaxAmount} className="bg-surface-100 px-3 py-1.5 rounded-full">
        ...
        {/* Payment Method */}
        <View className="mb-8">
          <Text className="text-[14px] font-bold text-surface-500 mb-4 ml-2 uppercase tracking-wider">Transfer To</Text>
            ...
            {methods.map((method, index) => {
              ...
              return (
                <Pressable
                  ...
                  style={!isSelected && index < methods.length - 1 ? { borderBottomColor: '#f1f5f9', borderBottomWidth: 1, borderRadius: 0 } : {}}
                >
                  <View className={`w-12 h-12 rounded-[16px] items-center justify-center mr-4 ${isSelected ? 'bg-brand-600' : 'bg-surface-100'}`}>
                    <Icon name={method.icon} size={24} color={isSelected ? "#fff" : "#64748b"} />
                  </View>
                  <View className="flex-1">
                    <Text className={`text-[16px] font-bold mb-0.5 tracking-tight ${isSelected ? 'text-brand-900' : 'text-surface-900'}`}>
                      {method.title}
                    </Text>
                    <Text className={`text-[13px] font-body ${isSelected ? 'text-brand-600' : 'text-surface-500'}`}>
                      {method.account}
                    </Text>
                  </View>
                  <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${isSelected ? 'border-brand-600 bg-brand-600' : 'border-surface-300 bg-transparent'}`}>
        ...
        {/* Receipt Summary */}
        <View className="mb-8">
          <Text className="text-[14px] font-bold text-surface-500 mb-4 ml-2 uppercase tracking-wider">Summary</Text>
          <View className="bg-white rounded-[24px] border border-surface-100 p-6 shadow-sm shadow-surface-200/30">
            <View className="flex-row justify-between items-center mb-4">
              ...
              <Text className="text-[15px] font-bold text-surface-900">GHS {numAmount.toFixed(2)}</Text>
            </View>
            ...
            <View className="h-[2px] bg-surface-200 w-full mb-5" style={{ borderStyle: 'dashed' }} />
            
            <View className="flex-row justify-between items-center">
              <Text className="text-[16px] font-bold text-surface-900">Total Deduction</Text>
              <Text className="text-[20px] font-heading font-black text-brand-600 tracking-tight">GHS {totalDeduction.toFixed(2)}</Text>
            </View>
          </View>
        </View>
      </ScrollView>
      ...
      >
        <View className="flex-1 justify-end" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          ...
          <View className="bg-white rounded-t-[32px] p-6 shadow-2xl" style={{ paddingBottom: Math.max(insets.bottom, 20) }}>
            ...
            <View className="flex-row justify-between items-center mb-6">
              ...
              <Pressable onPress={() => setShowAddMethod(false)} className="w-8 h-8 bg-surface-100 rounded-full items-center justify-center">
            ...
            <View className="flex-row gap-4 mb-8">
              ...
              >
                <View className={`w-12 h-12 rounded-full items-center justify-center mb-3 ${newMethodType === "momo" ? 'bg-brand-600' : 'bg-white'}`}>
                  <Icon name="smartphone" size={24} color={newMethodType === "momo" ? "#fff" : "#64748b"} />
                </View>
                <Text className={`text-[14px] font-bold ${newMethodType === "momo" ? 'text-brand-900' : 'text-surface-600'}`}>Mobile Money</Text>
              </Pressable>
              ...
              >
                <View className={`w-12 h-12 rounded-full items-center justify-center mb-3 ${newMethodType === "bank" ? 'bg-brand-600' : 'bg-white'}`}>
                  <Icon name="briefcase" size={24} color={newMethodType === "bank" ? "#fff" : "#64748b"} />
                </View>
                <Text className={`text-[14px] font-bold ${newMethodType === "bank" ? 'text-brand-900' : 'text-surface-600'}`}>Bank Transfer</Text>
              </Pressable>
            </View>
            ...
            <View className="mb-8">
              ...
              <TextInput
                placeholder={newMethodType === "momo" ? "e.g. 024 123 4567" : "Enter account number"}
      ...
      >
        <View className="flex-1 justify-end backdrop-blur-sm" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <View className="bg-white rounded-t-[40px] p-8 pb-12 h-[85%] shadow-2xl">
            ...
            <Text className="text-[15px] font-body text-surface-500 text-center mb-10 px-4">
              Enter your 4-digit security PIN to confirm the withdrawal of <Text className="font-bold text-surface-900">GHS {numAmount.toFixed(2)}</Text>.
            </Text>
