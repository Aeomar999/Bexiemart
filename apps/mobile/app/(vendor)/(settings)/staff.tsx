...
import { useVendorStaff, useCreateStaff, useUpdateStaff, useDeleteStaff, useToggleStaff } from "@/lib/hooks/use-vendor-staff";
    ...
    <View className="flex-1 bg-background">
                ...
                >
                  <View className="flex-row items-center justify-between mb-3">
                    <View className="flex-row items-center">
                      ...
                      <View>
                        <View className="flex-row items-center">
                          <Text className="text-[16px] font-bold text-foreground mr-2">{staff.name}</Text>
                          <View className={`px-2 py-0.5 rounded-full ${staff.role === 'Owner' ? 'bg-amber-100' : 'bg-muted'}`}>
                            <Text className={`text-[10px] font-bold ${staff.role === 'Owner' ? 'text-amber-700' : 'text-muted-foreground'}`}>
                              {staff.role}
                            </Text>
                          </View>
                        </View>
                  ...
                  {staff.role !== 'Owner' && (
                    <View className="flex-row items-center justify-between bg-background p-3 rounded-[12px] border border-border mt-2">
                      <Text className="text-[14px] font-medium text-muted-foreground">Account Access</Text>
                      <Switch
                        value={staff.active}
              ...
              <View>
                ...
                <View className="flex-row gap-2">
                  {ROLES.map(r => (
                    ...
                    >
                      <Text className={`text-[13px] font-bold ${role === r ? 'text-white' : 'text-muted-foreground'}`}>{r}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
      ...
      >
        <View className="flex-1 justify-end bg-black/40">
          ...
          <View className="bg-card rounded-t-[32px] p-6 pb-12">
            <View className="w-12 h-1.5 bg-accent rounded-full self-center mb-6" />
            <Text className="text-[20px] font-heading font-bold text-foreground mb-6">Manage Staff</Text>
            ...
            <View className="gap-3">
              ...
              >
                ...
                <View className="ml-4 flex-1">
                  ...
                  <Text className="text-[13px] font-body text-muted-foreground">Change name, email or role</Text>
                </View>
              ...
              >
                ...
                <View className="ml-4 flex-1">
                  ...
                  <Text className="text-[13px] font-body text-rose-500">Revoke access permanently</Text>
                </View>
