import { View, Text, ScrollView, Pressable, Switch, ActivityIndicator, Alert, Modal } from "react-native";
...
import { useVendorCoupons, useCreateCoupon, useUpdateCoupon, useDeleteCoupon, useToggleCoupon } from "@/lib/hooks/use-vendor-coupons";
        ...
        <ScrollView className="flex-1 px-5 pt-6 pb-12">
          ...
          <View className="bg-brand-600 rounded-[20px] p-5 mb-8 relative overflow-hidden">
            ...
            <View className="flex-row items-center justify-between mb-2">
              <View className="flex-row items-center">
                ...
                <Text className="text-[18px] font-heading font-black text-white">BexieMart Flash Sales</Text>
              </View>
              <Switch
                value={false}
                onValueChange={() => {}}
            ...
            <Text className="text-[14px] text-white/80 leading-relaxed">
              Opt-in to platform-wide flash sales. BexieMart will automatically apply a 15% discount to your top products during flash sale hours.
            </Text>
          </View>
          ...
          <View className="gap-3">
            {(coupons ?? []).map((promo: any) => (
              <View key={promo.id} className="bg-card rounded-[16px] border border-border p-4 flex-row items-center">
                ...
                <View className="flex-1">
                  <View className="flex-row items-center mb-1">
                    <Text className="text-[16px] font-heading font-black text-foreground mr-2">{promo.code}</Text>
                    <View className={`px-2 py-0.5 rounded-full ${promo.active ? 'bg-green-100' : 'bg-accent'}`}>
                      <Text className={`text-[10px] font-bold ${promo.active ? 'text-green-700' : 'text-muted-foreground'}`}>
        ...
        <View className="flex-1 justify-end bg-black/40">
          ...
          <View className="bg-card rounded-t-[32px] p-6 pb-12">
            ...
            <View className="gap-4">
              <Input label="Coupon Code" placeholder="e.g. SUMMER20" value={code} onChangeText={setCode} autoCapitalize="characters" />
              <Input label="Value" placeholder="e.g. 10% OFF or GHS 5" value={value} onChangeText={setValue} />
              <Button
