...
export default function VendorReviewsScreen() {
  ...
  const handleReply = () => {
    ...
    replyToReview.mutate({ id: replyModalId, reply: replyText }, {
      onSuccess: () => {
  ...
  const avgRating = (reviews ?? []).length
    ? ((reviews ?? []).reduce((sum: number, rev: any) => sum + rev.rating, 0) / (reviews ?? []).length).toFixed(1)
    : "0.0";
        ...
        <ScrollView className="flex-1 px-5 pt-6 pb-12">
          ...
          <View className="bg-card rounded-[24px] border border-border p-6 mb-6 items-center flex-row">
            <View className="items-center mr-6 border-r border-border pr-6">
              <Text className="text-[48px] font-heading font-black text-foreground">{avgRating}</Text>
              <View className="flex-row mb-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Icon key={star} name="star" size={16} color="#f59e0b" style={{ marginHorizontal: 2 }} />
                ))}
              </View>
              <Text className="text-[12px] text-muted-foreground">Based on {(reviews ?? []).length} reviews</Text>
            </View>
            <View className="flex-1 gap-2">
              {ratingDistribution.map((item) => (
                <View key={item.rating} className="flex-row items-center">
                  <Text className="text-[12px] font-bold text-muted-foreground w-3">{item.rating}</Text>
                ...
                <View className="flex-row justify-between items-start mb-3">
                  <View className="flex-row items-center">
                    <View className="w-10 h-10 rounded-full bg-accent items-center justify-center mr-3">
                      <Text className="text-[16px] font-bold text-muted-foreground">{review.customer?.charAt(0) || "?"}</Text>
                    </View>
                    <View>
                      <Text className="text-[15px] font-bold text-foreground">{review.customer}</Text>
                  ...
                  <View className="flex-row bg-amber-50 px-2 py-1 rounded-full items-center">
                    ...
                    <Text className="text-[12px] font-bold text-amber-600 ml-1">{review.rating}.0</Text>
                  </View>
                </View>
                ...
                {review.reply ? (
                  <View className="bg-background p-3 rounded-[12px] border border-border">
                    <View className="flex-row items-center mb-1">
                      <Icon name="corner-down-right" size={14} color="#64748b" style={{ marginRight: 6 }} />
                  ...
                  <Pressable
                    onPress={() => { setReplyModalId(review.id); setReplyText(""); }}
                    className="flex-row items-center justify-center py-2.5 bg-brand-50 rounded-[12px] border border-brand-100"
                  >
                    <Icon name="message-circle" size={16} color="#004CFF" style={{ marginRight: 6 }} />
      ...
      >
        <View className="flex-1 justify-end bg-black/40">
          ...
          <View className="bg-card rounded-t-[32px] p-6 pb-12">
            ...
            <Text className="text-[20px] font-heading font-bold text-foreground mb-6">Reply to Review</Text>
