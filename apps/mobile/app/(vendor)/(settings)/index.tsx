...
const SETTINGS_SECTIONS = [
  {
    ...
    items: [
      { id: "profile", icon: "store", label: "Store Profile", route: "/(vendor)/(settings)/profile", color: "#004CFF" },
      { id: "hours", icon: "clock", label: "Operating Hours", route: "/(vendor)/(settings)/hours", color: "#f59e0b" },
      { id: "staff", icon: "users", label: "Staff Management", route: "/(vendor)/(settings)/staff", color: "#10b981" },
    ]
  },
  {
    title: "Marketing & Feedback",
    items: [
      { id: "promotions", icon: "tag", label: "Promotions & Discounts", route: "/(vendor)/(settings)/promotions", color: "#ec4899" },
      { id: "reviews", icon: "star", label: "Customer Reviews", route: "/(vendor)/(settings)/reviews", color: "#f59e0b" },
    ]
  },
  {
    title: "Financials",
    items: [
      { id: "payment", icon: "credit-card", label: "Payment Methods", route: "/(vendor)/(settings)/payment", color: "#7c3aed" },
      { id: "taxes", icon: "file-text", label: "Taxes & Documents", route: "/(vendor)/(settings)/taxes", color: "#ec4899" },
    ]
  },
  {
    title: "Account & Preferences",
    items: [
      { id: "notifications", icon: "bell", label: "Notifications", route: "/(vendor)/(settings)/notification-settings", color: "#f59e0b" },
      { id: "dark_mode", icon: "moon", label: "Dark Mode", type: "toggle", color: "#1e293b" },
      { id: "security", icon: "shield", label: "Security", route: "/(vendor)/(settings)/security", color: "#059669" },
    ]
  },
  {
    title: "Support",
    items: [
      { id: "help", icon: "help-circle", label: "Help Center", route: "/(vendor)/(settings)/help", color: "#3b82f6" },
      { id: "contact", icon: "message-circle", label: "Contact Us", route: "/(vendor)/(settings)/contact", color: "#10b981" },
    ]
  }
];
      ...
      >
        ...
        <View className="bg-card rounded-[24px] p-5 flex-row items-center shadow-[0_10px_20px_rgba(0,0,0,0.03)] border border-border mb-8">
          ...
          <View className="flex-1">
            <Text className="text-[20px] font-heading font-bold text-foreground">{user?.name || "My Store"}</Text>
            <Text className="text-body-sm font-body text-muted-foreground">{user?.email || "Vendor Account"}</Text>
          </View>
          <Pressable 
