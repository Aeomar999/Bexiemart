...
const STAT_ITEMS = [
  { key: "totalProducts", label: "Total Products", icon: "package", color: "#3b82f6", bg: "#eff6ff" },
  { key: "totalOrders", label: "Total Orders", icon: "shopping-bag", color: "#10b981", bg: "#d1fae5" },
...
const QUICK_ACTIONS = [
  { id: "1", label: "Add\nProduct", icon: "plus", color: "#10b981", bg: "#ecfdf5", route: "/(vendor)/(products)/add" },
  { id: "2", label: "Withdraw\nFunds", icon: "dollar-sign", color: "#004CFF", bg: "#e0e7ff", route: "/(vendor)/(earnings)" },
  { id: "5", label: "Create\nReel", icon: "video", color: "#8b5cf6", bg: "#f3e8ff", route: "/(vendor)/add-reel" },
  { id: "4", label: "Store\nSettings", icon: "settings", color: "#64748b", bg: "#f1f5f9", route: "/(vendor)/(settings)" },
];
...
export default function DashboardScreen() {
  ...
  const { data: stats, isLoading: statsLoading, isError: statsError, refetch: refetchStats } = useVendorStats();
  ...
  return (
    <View className="flex-1 bg-background">
      {/* Header with Notification/Inbox */}
      <View 
        className="px-5 pb-2 flex-row items-center justify-end gap-3"
        style={{ paddingTop: (insets.top || 12) + 12 }}
      >
        <Pressable 
          style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]}
          className="w-10 h-10 rounded-full bg-card border border-border items-center justify-center relative"
        ...
        <View className="px-5 mb-8">
          ...
          <View className="bg-foreground rounded-[24px] p-6 border border-surface-800">
            <View className="flex-row justify-between items-start mb-6">
              <View>
                <Text className="text-muted-foreground font-body text-[13px] mb-1">Available Balance</Text>
