...
export default function RestaurantScreen() {
  ...
  const menuGroups = restaurant?.menu ?? (restaurant?.foodItems
    ? Object.entries(
  ...
  return (
    <View className="flex-1 bg-background">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: cartItemCount > 0 ? 100 : 40 }}>
        ...
        <View className="relative w-full justify-end bg-foreground" style={{ height: 260 }}>
          ...
          <View className="p-5 z-10">
            <Text className="text-white font-heading font-black text-[32px]">{restaurant.shopName}</Text>
            <Text className="text-white/90 font-body text-[14px] mt-1">{restaurant.description ?? "Restaurant"}</Text>
          </View>
        </View>
          ...
          <View className="flex-row justify-between">
            <View className="items-center">
              <View className="flex-row items-center">
                ...
                <Text className="text-[16px] font-bold text-foreground ml-1">{restaurant.city ?? restaurant.address ?? "Accra"}</Text>
              </View>
            ...
            <View className="items-center">
              <View className="flex-row items-center">
                ...
                <Text className="text-[16px] font-bold text-foreground ml-1">{restaurant._count?.foodItems ?? menuGroups.reduce((sum: number, g: any) => sum + g.items.length, 0)}</Text>
              </View>
            ...
            <View className="items-center">
              <View className="flex-row items-center">
                ...
                <Text className="text-[16px] font-bold text-foreground ml-1">{restaurant.hours?.length > 0 ? `${restaurant.hours[0].openTime ?? "09:00"}-${restaurant.hours[0].closeTime ?? "17:00"}` : "Open"}</Text>
              </View>
        ...
        <View className="border-b border-border bg-background sticky top-0 z-20 pb-2">
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, gap: 24 }}>
            {menuGroups.map((cat: any) => (
              <Pressable key={cat.category} onPress={() => setActiveCategory(cat.category)} className="pb-2 relative">
                <Text className={`text-[16px] font-bold ${activeCategory === cat.category ? 'text-orange-600' : 'text-muted-foreground'}`}>
                  {cat.category}
                </Text>
        ...
        <View className="px-5 pt-6">
          {menuGroups.map((cat: any) => (
            <View key={cat.category} style={{ display: activeCategory === cat.category ? 'flex' : 'none' }}>
              <Text className="text-[20px] font-heading font-bold text-foreground mb-4">{cat.category}</Text>
              <View className="gap-4">
                {cat.items.map((item: any) => (
                  <View key={item.id} className="bg-card rounded-[20px] p-4 border border-border flex-row justify-between">
                    <View className="flex-1 pr-4 justify-center">
                      <Text className="text-[16px] font-bold text-foreground mb-1 font-heading">{item.name}</Text>
                      <Text className="text-[13px] text-muted-foreground mb-2 leading-tight font-body" numberOfLines={2}>{item.description}</Text>
                      <Text className="text-[15px] font-bold text-brand-600 font-heading">GHS {item.price.toFixed(2)}</Text>
                    </View>
