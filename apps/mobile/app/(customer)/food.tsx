...
export default function FoodDeliveryScreen() {
  ...
  const { data: restaurantsData, isLoading } = useFoodRestaurants(selectedCategory ? { category: selectedCategory } : undefined);
  ...
  const PROMO_BANNERS = [
    { id: "1", title: "Free Delivery", subtitle: "On your first 3 food orders!", bgImage: "https://images.unsplash.com/photo-1504674900247-0877df9cc836?q=80&w=800&auto=format&fit=crop", color: "bg-orange-600" },
    { id: "2", title: "50% Off KFC", subtitle: "Valid until 8 PM today", bgImage: "https://images.unsplash.com/photo-1513104890138-7c749659a591?q=80&w=800&auto=format&fit=crop", color: "bg-rose-600" },
  ];
          ...
          <FlatList 
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  <View className={`w-16 h-16 rounded-2xl items-center justify-center mb-2 border ${isSelected ? 'bg-orange-500 border-orange-600' : 'bg-orange-50 border-orange-100'}`}>
                    <Icon name={item.icon} size={24} color={isSelected ? "#ffffff" : "#ea580c"} />
                  </View>
                  <Text className={`text-[13px] font-bold font-body ${isSelected ? 'text-orange-600' : 'text-muted-foreground'}`}>{item.name}</Text>
                </Pressable>
              );
            }}
          />
          ...
          <View className="mb-8">
                ...
                <View style={{ width: Dimensions.get("window").width, paddingHorizontal: 20 }}>
                  ...
                  >
                    ...
                    <View className="flex-1 p-5 justify-center">
                      <Text className="text-white font-heading font-black text-[24px] mb-1">{item.title}</Text>
                      <Text className="text-white/90 font-body text-[14px] mb-4">{item.subtitle}</Text>
            ...
            <View className="flex-row justify-center items-center mt-3 gap-1.5">
              {PROMO_BANNERS.map((_, i) => (
                <View key={i} className={`h-1.5 rounded-full ${i === activePromoIndex ? "w-4 bg-orange-500" : "w-1.5 bg-accent"}`} />
              ))}
            </View>
          </View>
        )}
        ...
        <View className="px-5 pb-10">
          ...
          {filteredRestaurants.length === 0 ? (
            <View className="items-center justify-center py-10">
              ...
              <Text className="text-[16px] font-bold text-muted-foreground mt-4">No restaurants found</Text>
              <Text className="text-[14px] text-muted-foreground mt-1">Try adjusting your search or filters.</Text>
            </View>
          ) : (
              ...
              {filteredRestaurants.map((restaurant: any) => (
                ...
                >
                  <View className="h-32 bg-accent items-center justify-center">
                    {restaurant.banner ? (
                      <Image source={{ uri: restaurant.banner }} style={{ width: '100%', height: '100%' }} contentFit="cover" />
                    ) : (
                  ...
                  <View className="p-4">
                    <View className="flex-row justify-between items-start mb-2">
                      <Text className="text-[16px] font-bold text-foreground font-heading">{restaurant.shopName}</Text>
                    </View>
                    ...
                    <View className="flex-row gap-4">
                      <View className="flex-row items-center gap-1.5">
                        ...
                        <Text className="text-[13px] font-bold text-muted-foreground">{restaurant.city ?? restaurant.address ?? "Accra"}</Text>
                      </View>
                      <View className="flex-row items-center gap-1.5">
                        <Icon name="shopping-bag" size={14} color="#64748b" />
                        <Text className="text-[13px] font-bold text-muted-foreground">{restaurant._count?.foodItems ?? 0} items</Text>
                      </View>
                    </View>
                  </View>
                </Pressable>
