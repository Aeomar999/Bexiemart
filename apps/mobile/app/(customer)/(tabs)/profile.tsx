...
const PROFILE_SECTIONS = [
  {
    ...
    items: [
      { id: "wallet", icon: "credit-card", label: "My Wallet", route: "/(customer)/wallet", color: "#f59e0b" },
      { id: "referrals", icon: "gift", label: "Refer & Earn", route: "/(customer)/referrals", color: "#ec4899" },
    ]
  },
  {
    title: "Account",
    items: [
      { id: "orders", icon: "shopping-bag", label: "Order History", route: "/(customer)/orders", color: "#004CFF" },
  ...
  },
  {
    title: "Settings",
    items: [
      { id: "notifications", icon: "bell", label: "Notifications", route: "/(customer)/notifications", color: "#f59e0b" },
      { id: "dark_mode", icon: "moon", label: "Dark Mode", type: "toggle", color: "#1e293b" },
      { id: "language", icon: "globe", label: "Language", value: "English", route: "/(customer)/settings/language", color: "#0ea5e9" },
    ]
  },
  {
    title: "Support",
    items: [
      { id: "help", icon: "help-circle", label: "Help Center", route: "/(customer)/help", color: "#3b82f6" },
      { id: "contact", icon: "message-circle", label: "Contact Us", route: "/(customer)/contact", color: "#10b981" },
    ]
  }
];
      ...
      >
        ...
        <View className="bg-card rounded-[24px] p-5 flex-row items-center shadow-[0_10px_20px_rgba(0,0,0,0.03)] border border-border mb-8">
          ...
          <View className="flex-1">
            <Text className="text-[20px] font-heading font-bold text-foreground">{user?.name || "Guest"}</Text>
            <Text className="text-body-sm font-body text-muted-foreground">{user?.email || "Login to access full features"}</Text>
          </View>
          <Pressable style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]} className="w-10 h-10 rounded-full bg-background items-center justify-center" onPress={() => router.push("/(customer)/edit-profile")}>
                    ...
                    onPress={() => {
                      if (item.route && item.route !== '#') {
                        if (item.route.includes('language')) {
                           Toast.show({ type: "info", text1: "Coming Soon", text2: "Language settings are not available yet." });
                        } else {
