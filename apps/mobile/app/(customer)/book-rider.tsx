...
const RIDER_TYPES = [
  { id: "bike", label: "Motorbike", icon: "truck", time: "10-15 min", price: 8.00, color: "#059669" },
  { id: "car", label: "Car", icon: "car", time: "5-10 min", price: 15.00, color: "#004CFF" },
  { id: "van", label: "Van", icon: "package", time: "15-20 min", price: 25.00, color: "#7c3aed" },
];
...
export default function BookRiderScreen() {
  ...
  const handleBook = () => {
    if (!pickup.trim() || !dropoff.trim()) {
      showPopup({ type: "error", title: "Missing locations", message: "Please select both pickup and drop-off locations." });
      return;
    }
    ...
    showPopup({ type: "success", title: "Looking for rider", message: "Searching for the nearest available rider." });
  ...
  return (
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top }}>
      ...
      <ScrollView className="flex-1 px-5 pt-6" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="always">
        
        {/* Location Box */}
        <View className="bg-card rounded-[24px] p-5 border border-border mb-6">
          <Text className="text-[14px] font-bold text-muted-foreground font-heading mb-2">Pickup Location</Text>
          ...
          <Text className="text-[14px] font-bold text-muted-foreground font-heading mb-2">Drop-off Location</Text>
        ...
        {/* Rider Selection */}
        <Text className="text-[16px] font-bold text-foreground font-heading mb-4 px-1">Choose Rider Type</Text>
        <View className="gap-3 mb-8">
          {RIDER_TYPES.map((rider) => {
            const isSelected = selectedRider === rider.id;
            return (
              <Pressable key={rider.id}
              ...
              >
                <View className={`w-12 h-12 rounded-full items-center justify-center`} style={{ backgroundColor: `${rider.color}20` }}>
                  <Icon name={rider.icon} size={22} color={rider.color} />
                </View>
                <View className="flex-1">
                  <Text className={`text-[16px] font-bold font-body ${isSelected ? 'text-brand-900' : 'text-foreground'}`}>{rider.label}</Text>
                  <Text className={`text-[13px] font-body ${isSelected ? 'text-brand-600' : 'text-muted-foreground'}`}>Arrives in {rider.time}</Text>
                </View>
                <Text className={`text-[16px] font-bold font-heading ${isSelected ? 'text-brand-700' : 'text-foreground'}`}>GHS {rider.price.toFixed(2)}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      <View className="px-5 py-4 bg-card border-t border-border" style={{ paddingBottom: Math.max(insets.bottom, 16) }}>
        <Pressable 
          ...
          onPress={handleBook}
        >
          <Text className="text-white font-bold text-[16px]">Book {RIDER_TYPES.find(r => r.id === selectedRider)?.label}</Text>
        </Pressable>
      </View>
      ...
      <Modal visible={!!selectingField} animationType="slide" transparent>
        <View className="flex-1 justify-end bg-black/40">
          <View className="bg-card rounded-t-[32px] p-6 h-2/3" style={{ paddingBottom: Math.max(insets.bottom, 20) }}>
            <View className="flex-row justify-between items-center mb-6">
              ...
              <Pressable onPress={() => setSelectingField(null)} className="p-2 bg-muted rounded-full">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {savedAddresses.map((addr: any) => (
                <TouchableOpacity key={addr.id} 
                  ...
                  onPress={() => handleSelectAddress(`${addr.name} - ${addr.address}, ${addr.city}`)}
                >
                  <View className="w-10 h-10 rounded-full bg-muted items-center justify-center">
                    <Icon name={addr.type === 'Home' ? 'home' : addr.type === 'Office' ? 'briefcase' : 'map-pin'} size={18} color="#64748b" />
                  </View>
                  <View className="flex-1">
                    ...
                    <Text className="text-[13px] text-muted-foreground mt-1">{addr.address}, {addr.city}</Text>
                  </View>
                </TouchableOpacity>
              ))}

              <View className="mt-8 pt-4 border-t border-border">
                <Text className="text-[14px] font-bold text-muted-foreground mb-3">Or enter manually</Text>
                <TextInput 
