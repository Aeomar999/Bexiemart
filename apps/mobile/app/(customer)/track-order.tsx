...
export default function TrackOrderScreen() {
  ...
  useEffect(() => {
    ...
    if (activeRide.status === "searching") {
      timer = setTimeout(() => {
        ...
        showPopup({ type: "success", title: "Rider Found", message: `${activeRide.riderType} driver Kwame Osei is on the way!`});
      }, 5000);
  ...
  if (!activeRide) {
    return (
      <View className="flex-1 bg-background items-center justify-center p-5">
        ...
        <Text className="text-[20px] font-heading font-black text-foreground mb-2">No Active Ride</Text>
        <Text className="text-[15px] font-body text-muted-foreground text-center mb-8">You don't have any active deliveries or rides currently in progress.</Text>
  ...
  const getStatusColor = () => {
    switch (activeRide.status) {
      ...
      case "on_the_way": return "bg-emerald-500";
      case "arrived": return "bg-brand-500";
    ...
    <View className="flex-1 bg-background">
      ...
      <View className="px-5 w-full absolute z-10" style={{ top: Math.max(insets.top, 12) + 12 }}>
        <View className="flex-row items-center justify-between">
          ...
          <Pressable style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]}
            className="h-12 px-4 rounded-full bg-card flex-row items-center justify-center border border-border shadow-sm gap-2"
            onPress={handleCancel}
          >
            <Icon name="x" size={16} color="#ef4444" />
            <Text className="text-[14px] font-bold text-rose-600 font-heading">Cancel</Text>
          </Pressable>
        </View>
      </View>
      ...
      <View className="absolute bottom-0 left-0 right-0">
        <View className="bg-card rounded-t-[32px] p-6 border-t border-border shadow-sm" style={{ paddingBottom: Math.max(insets.bottom, 24) }}>
          ...
          <View className="flex-row justify-between items-end mb-6">
            <View>
              {activeRide.status === "searching" ? (
                <Text className="text-[24px] font-heading font-black text-foreground">Locating...</Text>
              ) : activeRide.status === "arrived" ? (
                <Text className="text-[24px] font-heading font-black text-foreground">Arrived</Text>
              ) : (
                <Text className="text-[28px] font-heading font-black text-foreground">{timeRemaining} mins</Text>
              )}
              <Text className="text-[15px] font-body text-muted-foreground mt-1">
                {activeRide.status === "searching" ? "Finding nearest rider" : "Estimated arrival time"}
              </Text>
            </View>
            <View className={`px-3 py-1.5 rounded-md flex-row items-center gap-1.5 ${activeRide.status === 'searching' ? 'bg-orange-50 border border-orange-100' : 'bg-emerald-50 border border-emerald-100'}`}>
              <View className={`w-2 h-2 rounded-full ${getStatusColor()}`} />
              <Text className={`text-[12px] font-bold uppercase tracking-wider ${activeRide.status === 'searching' ? 'text-orange-600' : 'text-emerald-600'}`}>{getStatusText()}</Text>
            </View>
          </View>
          ...
          {activeRide.status !== "searching" ? (
            <View className="flex-row items-center justify-between bg-background p-4 rounded-[24px] border border-border mb-6">
              <View className="flex-row items-center gap-4">
                ...
                <View>
                  <Text className="text-[16px] font-bold text-foreground font-heading">{activeRide.driverName}</Text>
                  <Text className="text-[13px] text-muted-foreground font-body">{activeRide.driverVehicle}</Text>
                  <View className="flex-row items-center gap-1 mt-1">
                    <Icon name="star" size={12} color="#f59e0b" />
                    <Text className="text-[12px] font-bold text-muted-foreground">{activeRide.driverRating}</Text>
                  </View>
                </View>
              </View>
            ...
            <View className="flex-row items-center justify-center bg-background p-6 rounded-[24px] border border-border mb-6 border-dashed">
               <Text className="text-[14px] text-muted-foreground font-body">Matching you with a rider nearby...</Text>
            </View>
          )}
          ...
          <View className="flex-row items-start gap-4">
            ...
            <View className="flex-1">
              <Text className="text-[14px] font-bold text-foreground font-heading">Ride {activeRide.id}</Text>
              <Text className="text-[13px] text-muted-foreground font-body mt-0.5">{activeRide.pickup} to {activeRide.dropoff}</Text>
            </View>
          </View>
        </View>
      </View>
