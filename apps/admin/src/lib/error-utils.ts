...
export function getUserFriendlyErrorMessage(error: any, fallback: string): string {
  ...
  if (lowerMsg.includes("unauthenticated") || lowerMsg.includes("unauthorized") || lowerMsg.includes("invalid token")) {
    return "Your session has expired or is invalid. Please log in again.";
  }
  if (lowerMsg.includes("constraint violation") || lowerMsg.includes("unique") || lowerMsg.includes("duplicate")) {
  ...
  if (lowerMsg.includes("network error") || lowerMsg.includes("failed to fetch") || lowerMsg.includes("timeout")) {
  ...
  if (lowerMsg.includes("insufficient funds") || lowerMsg.includes("not enough balance")) {
  ...
  if (lowerMsg.includes("sql") || lowerMsg.includes("database") || lowerMsg.includes("internal server error")) {
    return fallback;
  }
